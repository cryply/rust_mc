...
fn cmd_info(input: PathBuf, stats: bool, head: Option<usize>) -> Result<()> {
    ...
    println!("│ Rows: {:^54}│", df.height());
    println!("│ Columns: {:^51}│", df.width());
    println!("└─────────────────────────────────────────────────────────────┘");
    ...
    for field in df.schema().iter_fields() {
        println!(
            "│ {:^24} │ {:^24} │",
            truncate_str(field.name(), 24),
            truncate_str(&field.dtype().to_string(), 24)
        );
    }
    println!("└──────────────────────────┴──────────────────────────┘");
    ...
    if stats {
        ...
        for name in df.get_column_names() {
            ...
            println!(
                ...
                null_count,
                truncate_str(&min_str, 14),
                truncate_str(&max_str, 14)
            );
        }
        println!("└──────────────────────────┴────────────┴────────────────┴────────────────┘");
    }
...
/// Format scalar value nicely (extract the inner value)

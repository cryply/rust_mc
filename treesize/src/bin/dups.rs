use std::{collections::{HashMap, VecDeque}, sync::{Mutex, OnceLock}};
        ...
        if TASKS.get().unwrap().lock().unwrap().len() > 0 {
            ...
            if metadata.is_dir() {
                ...
                if ts > 0 {
                    ...
                    let mut hashes = HASHES.get().unwrap().lock().unwrap();
                    let entry = hashes.entry(hash).or_insert(path.clone());
                    
                    if *entry != path {
                        println!("file {} and file {} are the same", &path, &entry);
                    }
                }
